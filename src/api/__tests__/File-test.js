...
describe('api/File', () => {
    ...
    describe('getUrl()', () => {
        ...
        test('should return correct file api url with id', () => {
            expect(file.getUrl('foo')).toBe(
                'https://api.box.com/2.0/files/foo',
            );
        });
    });
    ...
    describe('getDownloadUrl()', () => {
        test('should make xhr to get download url and call success callback', () => {
            ...
            const get = jest
                .fn()
                .mockReturnValueOnce(
                    Promise.resolve({ data: { download_url: 'bar' } }),
                );
            file.xhr = { get };
    ...
    describe('setFileDescription()', () => {
        ...
        test('should fail if the file object is bad', () => {
            file.xhr = jest.fn();
            return file
                .setFileDescription({}, 'foo', success, error)
                .catch(() => {
        ...
        test('should fail if we have insufficient permissions', () => {
            ...
            return file
                .setFileDescription(mockFile, 'foo', success, error)
                .catch(() => {
                    expect(file.xhr).not.toHaveBeenCalled();
                    expect(success).not.toHaveBeenCalled();
                    expect(error).toHaveBeenCalled();
        ...
        test('should merge the new file description in and execute the success callback', () => {
            ...
            file.xhr = {
                put: jest
                    .fn()
                    .mockReturnValueOnce(
                        Promise.resolve({ data: mockFileResponse }),
                    ),
            };
            ...
            return file
                ...
                .then(() => {
                    ...
                    expect(file.merge).toHaveBeenCalledWith(
                        'key',
                        'description',
                        'fo',
                    );
        ...
        test('should not call the error callback on failure when destroyed', () => {
            ...
            return file
                .setFileDescription(mockFile, 'bar', success, error)
                .then(() => {
                    expect(file.xhr.put).toHaveBeenCalled();
                    expect(file.merge).not.toHaveBeenCalled();
                    expect(error).not.toHaveBeenCalled();
    ...
    describe('getFile()', () => {
        ...
        test('should make xhr to get file when cached if missing fields', async () => {
            ...
            file.getCache = jest.fn().mockReturnValueOnce(cache);
            file.getCacheKey = jest.fn().mockReturnValueOnce('key');
            fields.findMissingProperties = jest
                .fn()
                .mockReturnValueOnce(['missing1', 'missing2']);
            fields.fillMissingProperties = jest
                .fn()
                .mockReturnValueOnce({ id: '123', foo: 'bar', missing: null });
            file.xhr = {
                get: jest
                    .fn()
                    .mockReturnValueOnce(
        ...
        test('should make xhr to get file and call success callback when missing fields', async () => {
            ...
            file.xhr = {
                get: jest
                    .fn()
                    .mockReturnValueOnce(Promise.resolve({ data: 'new file' })),
            };
        ...
        test('should make xhr to get file and not call success callback when destroyed', async () => {
            ...
            file.xhr = {
                get: jest
                    .fn()
                    .mockReturnValueOnce(
                        Promise.resolve({ data: { file: 'new file' } }),
                    ),
            };
        ...
        test('should make xhr to get file when forced to clear cache', async () => {
            ...
            fields.fillMissingProperties = jest
                .fn()
                .mockReturnValueOnce({ id: '123', foo: 'bar' });
            file.options = { cache };
            file.getCache = jest.fn().mockReturnValueOnce(cache);
            file.getCacheKey = jest.fn().mockReturnValueOnce('key');
            file.xhr = {
